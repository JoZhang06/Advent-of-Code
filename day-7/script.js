...
function sumOfDirectorySizeLessThanThreshold(node, threshold, totalSize) {
  ...
  for (let child of Object.values(node.children)) {
    totalSize = sumOfDirectorySizeLessThanThreshold(
      child,
      threshold,
      totalSize
    );
...
function findSmallestDirectoryGreaterThanSize(node, candidateNode, targetSize) {
  ...
  for (let child of Object.values(node.children)) {
    candidateNode = findSmallestDirectoryGreaterThanSize(
      child,
      candidateNode,
      targetSize
    );
...
function solve(part) {
  ...
  if (part === "part1") {
    ...
    console.log(
      findSmallestDirectoryGreaterThanSize(rootNode, null, moreSpaceNeeded).size
    );
  }
}
